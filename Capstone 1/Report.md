...
[County Business Patterns data](https://www.census.gov/data/tables/2015/econ/cbp/us-states-counties-pr-island-areas.html) is acquired by downloading 11 CSV files from the census website, selecting all optional columns to be included. Each is read into a dataframe and columns renamed to remove punctuation. 

[U.S. Population Migration Data](https://www.irs.gov/statistics/soi-tax-stats-migration-data) is aquired by downloading 4 CSV files and 7 ZIP files including 102 XLS files each. These files were further segmented into three different data formats for parsing, and four different filename formats for gathering for batch processing. Unlike the County Business Patterns data, the only year-identifying information in the migration data is found in the filenames. To add this information to the dataframes, dictionaries of years and lists of files were created and iterated over as tuples using the .items() method.

Missing values were initially identified by comparing the resulting dataframe shapes and uniques before concatenating them, typically by year. To determine how to handle changes in county districting from 2005 to 2015, the [US Census Bureau](https://www.census.gov/geo/reference/county-changes.html) is consulted. Based on this information, some counties had missing values filled with 0s and some were backfilled. The census bureau also witholds employee count information when values are low enough to reveal personally identifiable information (less than 10); these missing values were converted to 0s.
...
Data normalization " performed vertically on the raw values of population and businesses, but not horizontally across counties. Parishes and Counties are assumed by this model to behave similarly and have similar properties, but the variation in counties were not considered. Geographically larger counties would be expected to have less inter-county migration. Counties of high populations may be affected less or more by short-term population churn. County location relative to metropolis areas or "[Metropolitan  Statistical  Areas](https://www.census.gov/geo/reference/garm.html)" as the US Census defines them.
...
In this project, two government supplied data sources were used to predict county economic growth as measured in number of retail establishments. The data is cleaned and concatenated to give a socio-economic picture of each county. Visually and descriptively, the data is explored and shown to exhibit micro and macro trends across years and geography. 

In statistical exploration of the data, the fluctuation in correlation between population change and economic growth is assumed to be explained by the well-known economic recession. It is perhaps the case that in times of financial hardship, patterns in behavior are difficult to identify because people simple do not know what to do.

Pearson Correlation between Population Growth and Retail Establishment Growth

![Pearson Correlation between Population Growth and Retail Establishment Growth](images/pearson.png)

With the ten years of samples across 3,142 counties and county-equivalents the relationship between domestic migration and economic growth (as measured by number of retail establishments) is found to be weak. Predictive models plateau in accuracy in the mid-to-high 60% range, with a general failure to accurately learn the traing data, especially as pertaining to counties in the positive class. Models universally overestimated the shrinkage in number of retail establishments. 

A number of possible explanatory reasons are described and a few explored in the python notebooks for each step of the process. Simple online research corroborates the intuition that the correlation would be weak with surveys reporting the most common reasons to move being family and school, with "work" further down the list, and entrepreneurship unlisted (not to say that the hypothesis claimed the migrants were the entrepreneurs).